...
def test_scan_finds_calendars_opened_to_us_by_other_users() -> None:
    """Пожелание: «не кнопкой — синхронизируешь календарь, пробегись». У VK
    нет свойства «что мне расшарили», поэтому обходим пользователей сервера
    и берём то, что отвечает нам с правами."""
    principals_xml = '''<?xml version="1.0" encoding="utf-8"?>
<d:multistatus xmlns:d="DAV:"><d:response><d:href>/principals/corp.ru/</d:href>
<d:propstat><d:prop><d:resourcetype><d:collection/></d:resourcetype></d:prop>
<d:status>HTTP/1.1 200 OK</d:status></d:propstat></d:response>
<d:response><d:href>/principals/corp.ru/ivan/</d:href><d:propstat><d:prop>
<d:resourcetype><d:principal/></d:resourcetype></d:prop><d:status>HTTP/1.1 200 OK</d:status></d:propstat></d:response>
<d:response><d:href>/principals/corp.ru/petrov/</d:href><d:propstat><d:prop>
<d:resourcetype><d:principal/></d:resourcetype></d:prop><d:status>HTTP/1.1 200 OK</d:status></d:propstat></d:response>
</d:multistatus>'''
    petrov_xml = '''<?xml version="1.0" encoding="utf-8"?>
<d:multistatus xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav"><d:response>
<d:href>/principals/corp.ru/petrov/calendars/aaa/</d:href><d:propstat><d:prop>
<d:resourcetype><d:collection/><c:calendar/></d:resourcetype><d:displayname>Основной</d:displayname>
<d:owner><d:href>/principals/corp.ru/petrov/</d:href></d:owner></d:prop>
<d:status>HTTP/1.1 200 OK</d:status></d:propstat></d:response></d:multistatus>'''

    def answer(url, body, depth):
        if "current-user-principal" in body:
            return _propfind_response(_CURRENT_PRINCIPAL_XML)
        if "principal-collection-set" in body:
            raise RuntimeError("сервер не называет каталог принципалов")
        if url.rstrip("/").endswith("/principals/corp.ru"):
            return _propfind_response(principals_xml)
        if "petrov" in url:
            return _propfind_response(petrov_xml)
        raise RuntimeError("403 Forbidden")

    with patch("redmail.caldav_sync.caldav.DAVClient", return_value=_vk_like_client(answer)):
        found = CalDavSession(_vk_account()).scan_shared_calendars()

    assert [info.url for info in found] == ["https://calendar.corp.ru/principals/corp.ru/petrov/calendars/aaa/"]
    assert found[0].is_shared is True


def test_scan_stops_when_asked_and_uses_address_book_when_server_hides_users() -> None:
    petrov_xml = '''<?xml version="1.0" encoding="utf-8"?>
<d:multistatus xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav"><d:response>
<d:href>/principals/corp.ru/petrov/calendars/aaa/</d:href><d:propstat><d:prop>
<d:resourcetype><d:collection/><c:calendar/></d:resourcetype><d:displayname>Пётр</d:displayname>
<d:owner><d:href>/principals/corp.ru/petrov/</d:href></d:owner></d:prop>
<d:status>HTTP/1.1 200 OK</d:status></d:propstat></d:response></d:multistatus>'''
    ...
    def answer(url, body, depth):
        ...
        if "petrov" in url:
            return _propfind_response(petrov_xml)
        raise RuntimeError("403 Forbidden")  # каталог пользователей закрыт

    with patch("redmail.caldav_sync.caldav.DAVClient", return_value=_vk_like_client(answer)):
        session = CalDavSession(_vk_account())
        found = session.scan_shared_calendars(extra_logins=["petrov@corp.ru"])
        assert [info.name for info in found] == ["Пётр"]

        # Отмена: обход прекращается, ничего не собираем.
        assert session.scan_shared_calendars(extra_logins=["petrov@corp.ru"], stop=lambda: True) == []
