...
def is_shared_folder(path: str) -> bool:
    """Папка подписанного ящика коллеги. Такие папки не входят в фоновую
    офлайн-копию: чужой ящик может быть сколь угодно большим, а нужен он
    обычно на просмотр — содержимое запрашивается при открытии папки."""
