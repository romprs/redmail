...
5. **Доставка**: `scp` RPM с VM в `/d/111/`, затем с Windows на `.80:/var/tmp/` (НЕ в `/tmp`:
   там tmpfs и большие файлы портятся), там `rpm -K` должен сказать «digests ОК» (иначе
   переслать кусками по 16 МБ с `sha256sum -c`), затем `dnf -y install`, старый RPM удалить.
