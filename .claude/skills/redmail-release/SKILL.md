...
description: Выпуск сборки redmail — прогон тестов, бамп Release в spec с changelog, коммит и push, сборка RPM на VM RED OS, установка на VM и на .80, копия в D:\111\mail, проверка запуска. Использовать по команде /redmail-release или когда изменения готовы к сборке.
---
...
пользователем `test` в сессии `:1`. Копии пакета и инструкции для пользователя —
каталог обмена `D:\111\mail` (до 15.09.2026 клали в корень `D:\111`; в корень больше
ничего не кладём). Старую сборку оттуда убирать, чтобы не поставили прошлую версию.
...
5. **Доставка**: проще всего `bash scripts/deliver_release.sh <N>` — он ставит пакет на ВМ,
   кладёт копии в `E:\rrr\dist` и `D:\111\mail` со сверкой сумм и отправляет на .80.
   Вручную: `scp` RPM с VM в `/d/111/mail/`, затем на .80 ТОЛЬКО через
   `bash scripts/ship80.sh /d/111/mail/redmail-0.0.1-<N>.red80.x86_64.rpm` (у .80 сбоит ОЗУ:
...
## Перед каждой сборкой: ветки другой сессии

Над redmail параллельно работает сессия голосового помощника — её правки лежат в ветках
`origin/claude/*` (например, `claude/recipients-list`) и ставились на .80 руками. Каждый релиз
конвейера перекрывал их (список ФИО пропадал дважды за день). Поэтому перед сборкой:

```bash
git fetch origin && git branch -r --no-merged origin/main | grep claude/
```
Каждую такую ветку, перебазированную на main, влить `git merge --ff-only origin/<ветка>` (или обычным
merge, если не ff), прогнать тесты и только потом поднимать Release. В changelog — строка с именем ветки.

## Пути, в которых легко ошибиться

- Сборка под root кладёт пакет в `/root/rpmbuild/RPMS/x86_64/` — в `/home/test/rpmbuild/`
  лежат только старые сборки, запущенные под пользователем `test`. Ждать файл надо в `/root`.
- `scripts/ship80.sh` собирает куски в `/var/tmp/ship`, а готовый файл кладёт в
  `/var/tmp/<имя>.rpm` и каталог кусков удаляет. Ставить нужно из `/var/tmp/<имя>.rpm`.
- Готовый сценарий доставки — `scripts/deliver_release.sh <N>`: установка на ВМ,
  копии в каталог dist и в каталог обмена со сверкой сумм, отправка на .80,
  установка и перезапуск клиента.
...
В D:\111\mail держать только последний RPM: диск 29 ГБ забивался сборками (scp падал «No space left»).
